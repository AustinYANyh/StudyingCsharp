function colorEditor_loadData(data, thisDom)
{
    thisDom._pg_colorEditor_eb_colorTest27.loadData(data);
}

function colorEditor_cancel(thisDom)
{
    $(thisDom).parent().hide();
    //var parent=thisDom.parentElement.parentElement;
    //var pgData=thisDom._pg_colorEditor_eb_colorTest27;
    //orui_datagrid_edit(parent,pgData);
    orui_allowedClick($(thisDom).parent());
}

function colorEditor_beforeCancel(thisDom)
{

}

function colorEditor_confirm(thisDom)
{
    //$(thisDom).parent().hide();
    var parent=thisDom.parentElement.parentElement;
    var pgData=thisDom._pg_colorEditor_eb_colorTest27;
...
function colorEditor_beforeConfirm(thisDom) {

}

//初始化函数
function colorEditor_init(parentDom){
    //获得本窗口的包含DIV，e.g. thidDOM == thisDIV
    var _this = parentDom.querySelector("div");;
    GUI.Pages.colorEditor = _this;
    ...
    //假设有一个控件放在className为class1的div下面
        _div_class = _this.querySelector("._pg_colorEditor_eb_colorTest27").children[0];
    //必须在这个地方调用
    ...
    _this._pg_colorEditor_eb_colorTest27 = _div_class;
    GUI.Doms._pg_colorEditor_eb_colorTest27 = _div_class;
    ...
    {
        var __this = _this;
        colorEditor_loadData(data, __this);//注意有两个下划线__this
    }
    ...
    {
        var __this = _this;
        colorEditor_cancel(__this);
    }
    
    _this.beforeCancel = function () {
        var __this = _this;
        colorEditor_beforeCancel(__this);
    }
    _this.confirm = function () {
        var __this = _this;
        colorEditor_confirm(__this);
    }
    _this.beforeConfirm = function () {
        var __this = _this;
        colorEditor_beforeConfirm(__this);
    }
